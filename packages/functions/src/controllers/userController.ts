...
export const getUser = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  return sendSuccess(res, {
    ...
    usage: {
      totalsByProduct: usageSnap.docs.reduce((acc: Record<string, number>, doc) => {
        const data = doc.data();
...
export const unsuspendUser = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const db = admin.firestore();
  const {userId} = req.params;
